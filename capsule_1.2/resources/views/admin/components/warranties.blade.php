...
<div class="modal fade" id="deleteModal{{ $warranty->id }}" tabindex="-1" role="dialog"
    ...
    <div class="modal-dialog" role="document">
        <form method="POST" action="{{ route('admin.delete_warranty', ['id' => $warranty->id]) }}">
            @csrf
            @method('DELETE')
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="deleteModalLabel{{ $warranty->id }}">Delete Warranty?</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    Are you sure you want to delete this warranty (ID: {{ $warranty->id }})? This will also delete the
